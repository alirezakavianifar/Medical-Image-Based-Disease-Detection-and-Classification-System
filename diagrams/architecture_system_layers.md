...
graph TB
    subgraph "Tier 1 - Presentation Layer Web Browser"
        ...
        UI_Canvas["Clinical Canvas Viewer"]
        UI_Dashboard["Results Dashboard"]
    end

    subgraph "Tier 2 - Business Logic Layer Python Web API"
        Auth["Auth Controller JWT and PBKDF2"]
        Router["API Router FastAPI or Flask"]
        DICOM_Handler["DICOM Handler pydicom parser"]
        Report_Compiler["Report Compiler ReportLab PDF"]
    end

    subgraph "Tier 3 - AI Detection Layer CUDA GPU"
        Model_Loader["Model Loader Pre-trained Weights"]
        Inference["CNN Inference Engine PyTorch"]
        Spatial["Spatial Analyzer Bounding Box Detection"]
    end
    ...
    subgraph "Tier 4 - Database Layer SQLite"
        ...
        FileStore["DICOM File Cache Local Filesystem"]
    end
    ...
    DICOM_Handler -->|"SQL and file write"| DB_Images
